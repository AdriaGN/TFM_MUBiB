...
def divisio_dades() -> None:
    ...
    """
    # Verificar que la divisió demanada no s'hagi fet abans per evitar sobreescriure-la
    if parametres.RUTA_DIVISIONS.exists():
        logging.info(
            f"El model {parametres.NOM_MODEL} ja està dividit. Ignorant la divisió."
        )
    else:
        ...
        if len(mostres_netes) != 0:
            # Configuració de la llavor per reproductibilitat de les divisions
            ...
            logging.info(
                f"Dividint {len(mostres_netes)} mostres en {len(mostres_entrenament)} "
                f"mostres d'entrenament, {len(mostres_validacio)} mostres validació i"
